    ...
    return (
      ...
      >
        {isDarkMode ? (
          ...
          <Moon className="h-5 w-5 sm:h-6 sm:w-6 text-blue-200" />
        )}
      </button>
    );
  }
  ...
  return (
    ...
    >
      <div className="relative w-5 h-5 sm:w-6 sm:h-6">
        <Sun 
          className={cn(
            "absolute inset-0 w-5 h-5 sm:w-6 sm:h-6 text-yellow-400 transition-all duration-300",
    ...
    <>
        ...
        className={cn(
          ...
          isScrolled 
            ? isDarkMode 
              ...
              : "py-2 sm:py-3 bg-white/20 backdrop-blur-xl border-b border-white/20 shadow-2xl shadow-black/10"
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
          ...
          {/* Logo */}
          <a
            className="group relative flex items-center space-x-2 sm:space-x-3 text-lg sm:text-xl font-bold"
            href="#home"
          >
            <div className="relative">
              ...
              <Sparkles className="w-2 h-2 sm:w-3 sm:h-3 text-purple-400 absolute -top-0.5 -right-0.5 sm:-top-1 sm:-right-1 animate-pulse" />
            </div>
            <span className="relative">
              <span className={cn(
                "font-extrabold tracking-tight text-sm sm:text-base",
                isDarkMode ? "text-white" : "text-gray-900"
              )}>
                Mohammad
              </span>{" "}
              <span className="bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent text-sm sm:text-base">
                Mahdi
              </span>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navItems.map((item, key) => (
              <a
                ...
                href={item.href}
                className={cn(
                  "relative px-3 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-medium rounded-full transition-all duration-300 group",
          ...
          <div className="flex lg:hidden items-center space-x-2 sm:space-x-3">
            ...
            <button
              ...
              className={cn(
                "relative p-2 sm:p-3 rounded-full transition-all duration-300 z-50",
                "bg-white/10 backdrop-blur-sm border border-white/20",
                "hover:bg-white/20 hover:scale-110 focus:outline-none focus:ring-2 focus:ring-white/30",
          ...
          >
            ...
            <div 
              className={cn(
                "relative flex flex-col items-center justify-center h-full transition-all duration-500 ease-out px-4 sm:px-6",
                isMenuOpen ? "scale-100 opacity-100" : "scale-95 opacity-0"
              )}
            >
              <div className="flex flex-col space-y-4 sm:space-y-6 text-center w-full max-w-sm">
                {navItems.map((item, key) => (
                  <a
                    key={key}
                    href={item.href}
                    className={cn(
                      "group relative text-xl sm:text-2xl font-bold transition-all duration-300",
